...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".j5code"),
);
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .j5code > ambient J5CODE_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      ...
      delete output.J5CODE_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed j5code service (an
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // (gitignored) `.j5code` — see @t3tools/shared/devHome for why this must
    // outrank an ambient J5CODE_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.J5CODE_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.J5CODE_HOME ?? (yield* DEFAULT_T3_HOME);
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit T3 Code data directory; runtime state is stored under userdata (equivalent to J5CODE_HOME). Inside a git worktree this defaults to that worktree's own .j5code so dev state stays off the shared home.",
    ),
    Flag.optional,
