# Installs the J5 Code CLI from a GitHub Release archive on Windows. Needs
# only PowerShell 5.1+; no Node, npm, or compiler.
#
#   irm https://github.com/Jacksondr5/j5code/releases/latest/download/install.ps1 | iex
#
# J5 does not publish Windows archives yet (only darwin-arm64 and linux-x64);
# this script is kept pointed at J5 so it never installs upstream T3 Code.
#
# Environment:
...
#   J5CODE_HOME              J5 home directory (default: ~\.j5code); T3CODE_HOME is never read
#   T3CODE_INSTALL_BIN_DIR   where the j5.cmd shim goes (default: ~\.local\bin)
...
$t3Home = if ($env:J5CODE_HOME) { $env:J5CODE_HOME } else { Join-Path $HOME ".j5code" }
$binDir = if ($env:T3CODE_INSTALL_BIN_DIR) { $env:T3CODE_INSTALL_BIN_DIR } else { Join-Path $HOME ".local\bin" }

function Fail([string] $message) {
  Write-Error "j5 install: $message"
  exit 1
}
...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}J5 Code$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
if (-not $version) {
  ...
  $releases = Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/releases?per_page=100" -Headers @{ "User-Agent" = "j5-install" }
...
if ($version -match '-preview\.') {
  Write-Warning "j5 $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set T3CODE_CHANNEL=stable (the default) for a supported build."
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    [Console]::Error.WriteLine("  ${muted}Installing$reset J5 Code $bold$version$reset`n")
    Step "Downloading..."
    try {
      ...
      if ($status -eq 404) {
        Fail "j5 $version has no release archive for win32-$arch; J5 publishes CLI archives for macOS (Apple silicon) and Linux x64 only"
      }
      throw
    }
...
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "  Add $binDir to your PATH, then run ${bold}j5$reset.`n"
} else {
  Write-Host "  Run ${bold}j5$reset to get started.`n"
}
