...
| Task                            | Command                               |
| ------------------------------- | ------------------------------------- |
| Install and start               | `npx j5code@latest service install`   |
| Inspect status and log location | `npx j5code@latest service status`    |
| Update or repair                | `npx j5code@latest service update`    |
| Stop and remove from startup    | `npx j5code@latest service uninstall` |

Uninstalling the service leaves your projects, threads, and settings intact.

Install and update use the version of the CLI you invoke. For nightly, use
`npx j5code@nightly service update`; replace `nightly` with an exact version to pin
...
Start with `j5code service status` on the host. It prints the log path and, on Linux,
...
identity. Without administrator access, run `j5code serve` in a terminal and keep
that session open.
