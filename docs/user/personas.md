...
A conversation on any harness can propose a crew, and approving the roster is meant to be the only confirmation you give. Codex and Claude never add a step. Some harnesses still ask first or refuse:

- **OpenCode** and **Pi** show their own approval prompt before the roster in every mode except **Full access**.
...
- A conversation running as a saved persona with read-only access cannot propose a crew on Cursor, OpenCode, Grok, Antigravity, or ACP registry harnesses, and asks first on Pi.
