...
The agent uses the conversation to propose a roster above the composer. Each member has a responsibility, a reason for joining, and instructions. Members may use saved personas or be custom agents created for this task. You can remove members or add a custom seat with a name and instructions, then approve or decline. Custom members use the Captain's provider, model, and effective access mode; saved personas use their own configured runtime policy. No special Captain persona is needed, and one Captain can coordinate several crews in the same conversation. Before you approve, each member shows its resolved harness, model, reasoning level, and access. Changing the roster refreshes these details. If the configuration changes before launch, review the refreshed settings and approve again.
