...
The thread's agent, whichever saved agent or model it is running as, receives your brief together with instructions to compose a crew rather than do the work itself. It reads the saved-agent library and proposes a roster: one seat per agent with a reason. The proposal appears above the composer in that thread, like a planning question. Remove seats you do not want, or add a seat: pick **Custom agent** and give it a name and instructions for an agent that has no saved definition (instructions are required; it runs on the Captain's provider and model with the Captain's access mode, not a saved agent's sandbox), or pick a saved agent from the library and add instructions if it needs them. Then **Approve** or **Decline**. Approval starts every seat as a peer under the Captain with your brief and the roster, and the Captain is told the decision in its thread, where it shows as a card listing the crew and its seats; click a seat to open its thread. Your `/crew` message shows as the brief you wrote, with the instructions the command sent alongside it one click away. Seats run with their own agent's permissions once you approve them, so a read-only Captain can command agents that write.
