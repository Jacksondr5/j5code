...
Persona instructions describe intended behavior. They do not grant permissions or guarantee that an agent obeys them. The displayed runtime policy reflects supported provider controls; unsupported modes appear as **Blocked**. **User approved** uses Codex or Claude's normal permission prompts, allowing you to approve operations beyond the workspace. Skill installation moved to the **Settings → Skills** terminal wizard; the retired Skill Manager persona stays resolvable so earlier chats keep working. A constrained agent cannot delegate to a user-approved agent to gain broader permissions.
