...
```bash
npx j5code serve --host <private-ip>
```
...
```bash
npx j5code serve --tailscale-serve
```

For an already-running server:

```bash
npx j5code pair --tailscale
```
...
`--tailscale-serve-port`. See `npx j5code pair --help` for other pairing options.
...
management is available through `npx j5code auth --help`.
...
On a command-line host, `j5code connect unlink` disables exposure while retaining
your login; `j5code connect logout` also clears that login. Background-service
...
Run `j5code connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `j5code service status` and read the displayed log. If it disappears
...
| `auth_invalid` or `invalid_bearer`                        | Run `j5code connect login`. If credentials were revoked, run `j5code connect logout`, then `j5code connect` again. Restart the server after signing in. |
...
foreground server, stop it and run `j5code serve` again with your usual options.
