...
- **Orchestrator MCP server** (`orchestratorMcp.ts`): agents orchestrate agents via app-owned tools — `delegate_task` (spawn a child thread on *any* provider, durable result), `task_status/cancel`, `create_threads`, `t3_thread_start/list/read/send/wait/interrupt` (send modes auto/queue/steer/restart), scheduled tasks, per-session scoped bearer creds. Their docs muse about exactly our product: "tools that let agents spawn subagents of other providers powered by the T3 Orchestrator."
