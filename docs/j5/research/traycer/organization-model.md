...
**Why this matters for us:** they clearly hit a scaling wall with everything in one doc and split artifact bodies out. The lesson they did _not_ apply is that **chat messages were left behind in the root doc** — see the [performance deep dive](./performance.md). Splitting bodies into rooms is the correct pattern; it should have been applied to messages too.
...
- **`roleClaims`** in the epic record — agent self-designated roles (see the [A2A deep dive §15](./agent-to-agent.md)).
