...
Studied at commit `d407fb7` ("PR Group Dashboard: measure agent-run PR state instead of asking"), 2026-08-14. Companion to `../pr-group/` (the playbooks; this repo is the measurement half). ~3,150 lines, zero dependencies — Node 22 built-in SQLite, server-rendered HTML, no build step. Runs as a user LaunchAgent on `localhost:7317`, self-polling: **agent liveness every 20s, GitHub every 60s**, page reloads every 15s.
