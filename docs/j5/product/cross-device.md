...
- 2026-09-24 — names across peer servers (PR #198): AC17 lists a Squadron's name beside its id; AC23 added: a delivery to a peer server carries the sender's display name for the receiving client's timeline, bounded and read by the client identity lookup alone. The origin Squadron's name is not carried; nothing reads it.
