...
composer launch), its authority policy sets the child's runtime mode, and a read-only persona
parent cannot spawn a write-capable child — the same escalation rule `delegate_task` applies to
saved-agent subagents. Disabled, removed, and unknown agents refuse before creation. A plain spawn without
`agent` is unchanged.
...
Result: `proposal_id`, `status` (`open`, `approved`, `auto_approved`, `declined`),
...
policy, so a read-only Captain may command writing seats once a person approved them; the
read-only-parent rule that governs agent-initiated `spawn_agent` does not apply to approved seats.
There is no auto-approval; `runbook_declared` and `auto_approved` were withdrawn until runbooks
exist (the column and status literal remain for that future).
