...
16. An agent whose Role file changed since it was spawned shows a drift indicator.
...
- 2026-09-08 — personas implement Roles as a user-authored YAML library: definitions load from environment folders or imports, each launch pins an immutable snapshot and a primary/fallback route (the current shape of the allowlist), and Settings manages the library; the persona contract is the implementation contract ([contract](../agent-personas/index.md), [record](../../worklog/2026-09-08-user-authored-role-library.md)).
- 2026-09-14 — the Crew clause of AC16 retired: Crews are composed from a proposed roster and have no definition files to drift ([Crews](crews.md)).
