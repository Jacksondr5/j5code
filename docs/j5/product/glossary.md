...
| **persona**            | A saved definition in the library (Settings → Personas): instructions, model routes, runtime policy, declared handoff artifact. An agent may run as a persona; a persona is not itself running. "Saved agent" is retired vocabulary.                                                                                                                                                                                                                                               |
| **handoff artifact**   | The versioned document one agent writes for another: a persona's declared `outputArtifact`, stored under `handoffs/` in the project's artifacts and read by the next agent. Never a context handoff. Owner: [agent-personas/index.md](agent-personas/index.md#artifact-contracts).                                                                                                                                                                                                 |
| context handoff        | Upstream's mechanism, not a J5 concept: the automatic transfer of a budgeted slice of a thread's history into a new provider conversation of the same thread when it changes provider or model, forks, or restarts. Never a handoff artifact. Owner: upstream's [portable handoffs](../../user/portable-handoffs.md).                                                                                                                                                              |
