...
## 1. Fork & setup app ✅ *(complete 2026-08-15 — plan + evidence at `../fork-setup-plan/`)*
...
Define agent types, easily spawnable, configurable prompts, "soul" (SOUL.md-style identity). Prior art: Claude `.claude/agents/*.md`, Traycer role claims (runtime dedup half), T3 v2 role labels (one-sentence prompt prefix). The *definition layer* is ours alone — no one has it.

## 4. PR / Agent dashboard *(scope expanded by Jackson; 4a PR pane opening 2026-08-17 for human engineers)*
...
## 5. Cross-machine fleets *(deliberately deferred)*

Includes a broader cross-*person* A2A story. Jackson's call: both T3 and Traycer will likely make cross-machine/cross-person advances in the coming months — wait and see before making our move. Keep the door open architecturally (routing above the environment boundary; nothing that assumes single-host A2A forever).
