...
Check that cwd is a j5code checkout containing `apps/server/src/j5/playbook-definitions/Yaml.ts`. If missing, stop before writing: "Wrong branch or checkout: apps/server/src/j5/playbook-definitions/Yaml.ts is missing. Use a checkout containing the Playbook compiler."
...
- Custom publication: [examples/publication.yaml](examples/publication.yaml), including preparation and a report-based approval gate.

Copy their YAML structure. Do not copy the built-in Development implementation: its validation and repair operations are unavailable to custom YAML.
...
- Allow optional nonempty `label`, `tasks`, integer `visitLimit >= 1` (default 1), `outcome: completion|review` (default completion), and `evidence`, `approvals`, `capabilities` arrays of nonempty strings.
...
- For code tasks, set `operation: metadata|commit|push|draft|feedback` and omit agent/persona: "code tasks require one registered operation".
- Publishing requires exactly one contiguous sequence: `metadata` → gate → `commit` → `push` → `draft`, each operation in its own phase. Preparation selects exactly one developer report phase (one report task) through `evidence`. The report must run before preparation. Include preparation in gate evidence. No incoming transition may bypass this order; only the gate's `approve` enters commit. Draft may complete or continue to review/rework. Every update repeats preparation and approval and appends a commit to the same draft PR.
- Use `operation: feedback` after draft publication to collect read-only GitHub evidence for a Herald/reviewer task; bind that phase through `evidence`. An explicit human finish/refresh gate can route `request_changes` back to collection when reviews are pending. Collection limits are evidence gaps, not proof that review is complete.
- Route `request_changes` and `changed` back to development or preparation, with enough visits for the full loop. Preparation captures the exact code and initial publication text; humans can edit text at the gate. A changed candidate requires fresh preparation and approval. `__workspace` is generated automatically; `workspace`, `validation`, and `repair_capacity` operations are built-in-only.
