...
import { readPlaybookEntries, readPlaybookThreadParent } from "./SidebarRead.ts";
import { runPlaybookMigrations } from "./Migrations.ts";

const encodeJson = Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown));

it.effect(
  "lists bounded playbook titles, prioritizes gates and searches within the selected Squadron without loading evidence",
  () =>
    Effect.gen(function* () {
      ...
      const first = yield* readPlaybookEntries("selected", "", 0);
      ...
      const second = yield* readPlaybookEntries("selected", "", 50);
      assert.equal(second.runs.length, 2);
      assert.equal(second.hasMore, false);
      assert.equal(new Set([...first.runs, ...second.runs].map((run) => run.id)).size, 52);
      const search = yield* readPlaybookEntries("selected", "CHANGE 51", 0);
      assert.deepEqual(
        search.runs.map((run) => run.id),
        ["run-51"],
      );
      assert.equal((yield* readPlaybookEntries("selected", "Change 52", 0)).runs.length, 0);
      assert.equal((yield* readPlaybookEntries("", "Change 52", 0)).runs.length, 1);
      const playbookThreadId = `thread:pb:${"a".repeat(64)}`;
      yield* sql`INSERT INTO j5_playbook_actions(
        ...
        'completed', 1000, 'input-hash', ${`${playbookThreadId}/provider-run`}
      )`;
      assert.deepEqual(yield* readPlaybookThreadParent(playbookThreadId), {
        runId: "run-0",
        squadronId: "selected",
        title: "Change 0",
      });
...
it.effect("uses scoped ordering indexes for a 2,000-run fixture", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.forEach(
      ...
      (index) => sql`INSERT INTO j5_playbook_runs(
        id, definition_id, definition_version, definition_hash, squadron_id,
        project_id, title, phase, gate_revision, activity_at, status_priority,
        revision, read_version, status, payload
      ) VALUES (
