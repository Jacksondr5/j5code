...
interface RunSnapshot {
  ...
  readonly interruptions?: Run["interruptions"];
...
const snapshotOf = (
  ...
  ...(run.interruptions === undefined ? {} : { interruptions: run.interruptions }),
...
export const makeStore = Effect.gen(function* () {
  ...
  const valuesFor = Effect.fn("PlaybookStore.valuesFor")(function* (hashes: ReadonlyArray<string>) {
    if (hashes.length === 0) return new Map<string, unknown>();
    const rows = yield* sql<{ hash: string; payload: string }>`SELECT hash, payload
      FROM j5_playbook_values WHERE hash IN ${sql.in([...new Set(hashes)])}`;
    return new Map(rows.map((row) => [row.hash, JSON.parse(row.payload) as unknown]));
  });

  const hydrate = Effect.fn("PlaybookStore.hydrate")(function* (
    ...
    const identities = currentIdentities
      ? yield* sql<{ id: string; identity: string | null }>`SELECT id, identity
          FROM j5_playbook_actions WHERE run_id=${snapshot.id}`
      : [];
  ...
  const detail = Effect.fn("PlaybookStore.detail")(function* (id: string) {
    ...
    return yield* decodeDetail({
      ...
      ...(snapshot.interruptions === undefined ? {} : { interruptions: snapshot.interruptions }),
  ...
  const readVersion = Effect.fn("PlaybookStore.readVersion")(function* (id: string) {
    const rows = yield* sql<{ readVersion: number }>`SELECT read_version AS readVersion
      FROM j5_playbook_runs WHERE id=${id}`;
    if (!rows[0])
      return yield* new PlaybookError({ code: "not_found", detail: `Unknown playbook ${id}` });
    return rows[0].readVersion;
  });

  const artifact = Effect.fn("PlaybookStore.artifact")(function* (
    runId: string,
    artifactId: string,
  ) {
    const rows = yield* sql<{
      ...
      FROM j5_playbook_artifacts a
      JOIN j5_playbook_values v ON v.hash=a.hash
      WHERE a.run_id=${runId} AND a.id=${artifactId}`;
    const row = rows[0];
    if (!row)
      return yield* new PlaybookError({ code: "not_found", detail: "Unknown playbook artifact" });
  ...
  const save = Effect.fn("PlaybookStore.save")(function* (
    ...
    for (const action of run.actions) {
      ...
      if (
        ...
        prior.predecessorActionId === action.predecessorActionId
      )
        continue;
      yield* sql`INSERT INTO j5_playbook_actions(
          id, run_id, phase, revision, task, attempt, kind, adapter, status,
          deadline, input_hash, result_artifact_id, identity, predecessor_action_id
        ) VALUES (
          ...
          ${action.externalIdentity ?? null}, ${action.predecessorActionId ?? null}
        ) ON CONFLICT(id) DO UPDATE SET
          status=excluded.status,
          result_artifact_id=excluded.result_artifact_id,
          identity=coalesce(j5_playbook_actions.identity, excluded.identity),
          predecessor_action_id=coalesce(
            j5_playbook_actions.predecessor_action_id, excluded.predecessor_action_id
          )
        WHERE j5_playbook_actions.status IS NOT excluded.status
          ...
          OR (j5_playbook_actions.predecessor_action_id IS NULL
            AND excluded.predecessor_action_id IS NOT NULL)`;
    }
    ...
    yield* sql`INSERT INTO j5_playbook_runs(
        ...
        revision=excluded.revision, read_version=j5_playbook_runs.read_version+1,
        status=excluded.status, payload=excluded.payload,
        watch_candidate=excluded.watch_candidate`;
    yield* sql`INSERT INTO j5_playbook_history(run_id, revision, command_id, payload)
      VALUES (${run.id}, ${run.revision}, ${commandId}, ${canonical(event)})`;
    return { payload, run: responseRun };
  });

  const command = Effect.fn("PlaybookStore.command")(function* (
    input: {
    ...
    return yield* sql.withTransaction(
      Effect.gen(function* () {
        ...
        const receipts = yield* sql<{ input_hash: string; payload: string }>`SELECT
          input_hash, payload FROM j5_playbook_receipts WHERE command_id=${input.commandId}`;
        if (receipts[0]) {
        ...
        if (input.claim) {
          const owned = yield* sql`SELECT id FROM j5_playbook_actions
  ...
  const receipt = Effect.fn("PlaybookStore.receipt")(function* (
    commandId: string,
    receiptInput: unknown,
  ) {
    const rows = yield* sql<{ input_hash: string; payload: string }>`SELECT input_hash, payload
      FROM j5_playbook_receipts WHERE command_id=${commandId}`;
  ...
  const claim = Effect.fn("PlaybookStore.claim")(function* (
    actionId: string,
    owner: string,
    now: number,
  ) {
    const rows = yield* sql<{ generation: number }>`UPDATE j5_playbook_actions
      ...
      WHERE id=${actionId} AND status IN ('pending','claimed')
        ...
        AND EXISTS (SELECT 1 FROM j5_playbook_runs r WHERE r.id=run_id AND r.status='running')
      RETURNING generation`;
    return rows[0] ? ({ actionId, owner, generation: rows[0].generation } satisfies Claim) : null;
  });

  const identity = Effect.fn("PlaybookStore.identity")(function* (
    claim: Claim,
    value: string,
    now: number,
  ) {
    const current = yield* sql<{ runId: string; identity: string | null }>`SELECT
      run_id AS runId, identity FROM j5_playbook_actions
    ...
    const rows = yield* sql<{ runId: string }>`UPDATE j5_playbook_actions SET identity=${value}
      WHERE id=${claim.actionId} AND owner=${claim.owner} AND generation=${claim.generation}
    ...
    yield* sql`UPDATE j5_playbook_runs SET read_version=read_version+1, activity_at=${now}
      WHERE id=${rows[0].runId}`;
  });

  const renew = (claim: Claim, now: number) => sql`UPDATE j5_playbook_actions
    SET lease_until=${now + 60_000}
    WHERE id=${claim.actionId} AND owner=${claim.owner} AND generation=${claim.generation}
      AND lease_until>${now} RETURNING id`;
  const release = (claim: Claim) => sql`UPDATE j5_playbook_actions SET owner=NULL, lease_until=NULL
    WHERE id=${claim.actionId} AND owner=${claim.owner} AND generation=${claim.generation}`;

  const activeMaxSequence = Effect.fn("PlaybookStore.activeMaxSequence")(function* () {
    const rows = yield* sql<{ maximum: number | null }>`SELECT max(creation_sequence) AS maximum
      FROM j5_playbook_runs WHERE status IN ('running','restarting','cancelling')`;
    return rows[0]?.maximum ?? 0;
  });
  const runningBatch = Effect.fn("PlaybookStore.runningBatch")(function* (
    ...
    return yield* sql<SchedulingRecord>`SELECT id, definition_id AS definitionId,
      ...
      FROM j5_playbook_runs WHERE status='running' AND creation_sequence>${afterSequence}
      ORDER BY creation_sequence LIMIT ${Math.min(100, Math.max(1, limit))}`;
  });
  const activeBatch = Effect.fn("PlaybookStore.activeBatch")(function* (
    afterSequence: number,
