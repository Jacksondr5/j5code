import { BoardPage, TimelinePage, type TimelineEntry } from "@j5/playbook-contracts/observability";
...
const actionFields = (action: ObservedAction, observation?: PlaybookObservation) => ({
  actionId: action.id,
  task: action.task,
...
const changedBlockedState = (previous: PlaybookObservation, current: PlaybookObservation) =>
...
export function deriveTimelineRevisions(
  observations: ReadonlyArray<PlaybookObservation>,
  ...
  for (let index = 0; index < ordered.length; index++) {
    ...
    if (observation.source === "history") {
      ...
      if (observation.eventType === "result" && eventAction) {
        ...
        observation.eventType === "server_interrupted" &&
        observation.status === "blocked" &&
        observation.failureCategory === "server_interrupted"
      ) {
        add("interrupted", { ...optional("cause", observation.stateCause) });
        commandSupported = true;
      } else if (
        observation.eventType === "resume" &&
        previous?.failureCategory === "server_interrupted" &&
      ...
      for (const action of created) {
        add(
          "action_queued",
          {
            ...
            ...actionFields(action),
            ...(action.predecessorActionId ? { replacesActionId: action.predecessorActionId } : {}),
          },
...
export const readBoard = Effect.fn("Playbook.readBoard")(function* (
  squadronId: string,
  query: string,
  offset: number,
  limit = 24,
    ...
    Effect.gen(function* () {
      ...
      const counts = yield* sql.unsafe<{ total: number; waitingApprovalCount: number }>(
        `SELECT count(*) AS total,
          ...
          FROM j5_playbook_runs${filter.sql}`,
        filter.parameters,
      );
...
export const readTimeline = Effect.fn("Playbook.readTimeline")(function* (
  runId: string,
  before: number | null,
  limit = 50,
) {
  ...
  return yield* sql.withTransaction(
    Effect.gen(function* () {
      const heads = yield* sql<{ headRevision: number; readVersion: number }>`SELECT
        revision AS headRevision, read_version AS readVersion FROM j5_playbook_runs WHERE id=${runId}`;
      const head = heads[0];
      if (!head)
        return yield* new PlaybookError({ code: "not_found", detail: `Unknown playbook ${runId}` });
      const boundedLimit = Math.min(100, Math.max(1, limit));
      const rows = yield* sql.unsafe<PlaybookObservation>(
        `SELECT ${observationColumns} FROM j5_playbook_observations
      ...
      const predecessor = yield* sql.unsafe<PlaybookObservation>(
        `SELECT ${observationColumns} FROM j5_playbook_observations
      ...
      >(
        `SELECT a.id, a.revision, a.phase, a.task, a.attempt, a.kind,
          o.visit AS creationVisit, a.predecessor_action_id AS predecessorActionId
          FROM j5_playbook_actions AS a
          LEFT JOIN j5_playbook_observations AS o
      ...
      const verdictRows =
        artifactIds.length === 0
          ...
          FROM j5_playbook_artifacts AS a JOIN j5_playbook_values AS v ON v.hash=a.hash
