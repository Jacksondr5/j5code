...
import { restartPresentation } from "./Definition.ts";
...
it("interrupts active work and appends one replacement per unfinished action on Resume", () => {
  const entered = decide(reviewInitial, { type: "enter" }, reviewDefinition, 0);
  const actions = entered.actions.map((action, index) => ({
    ...action,
    externalIdentity: `thread:pb:${index}/run:${index}`,
  }));
  const running = { ...entered, actions };
  const interrupted = decide(running, { type: "server_interrupted" }, reviewDefinition, 4_000_000);
  assert.equal(interrupted.status, "blocked");
  assert.equal(interrupted.failureCategory, "server_interrupted");
  assert.equal(interrupted.recovery, "resume");
  assert.deepEqual(interrupted.interruptions, [
    { generation: 1, interruptedAt: 4_000_000, actionIds: actions.map((action) => action.id) },
  ]);
  assert.isTrue(interrupted.actions.every((action) => action.status === "blocked"));
  assert.strictEqual(
    decide(interrupted, { type: "server_interrupted" }, reviewDefinition, 5_000_000),
    interrupted,
  );

  const resumed = decide(interrupted, { type: "resume" }, reviewDefinition, 8_000_000);
  const replacements = resumed.actions.slice(-2);
  assert.equal(resumed.status, "running");
  assert.deepEqual(resumed.visits, entered.visits);
  assert.lengthOf(replacements, 2);
  assert.isTrue(
    replacements.every(
      (action, index) =>
        action.predecessorActionId === actions[index]!.id &&
        action.attempt === actions[index]!.attempt &&
        action.deadline === 9_800_000 &&
        action.externalIdentity === undefined,
    ),
  );
  assert.equal(resumed.interruptions?.[0]?.resumedAt, 8_000_000);

  const interruptedAgain = decide(
    resumed,
    { type: "server_interrupted" },
    reviewDefinition,
    8_100_000,
  );
  assert.equal(interruptedAgain.interruptions?.at(-1)?.generation, 2);
  const resumedAgain = decide(interruptedAgain, { type: "resume" }, reviewDefinition, 9_000_000);
  assert.lengthOf(resumedAgain.actions, 6);
  const cancelling = decide(resumedAgain, { type: "cancel" }, reviewDefinition, 9_000_001);
  assert.isTrue(
    cancelling.actions
...
it.effect("persists interruption and replays duplicate Resume commands", () =>
  Effect.gen(function* () {
    yield* runPlaybookMigrations();
    const store = yield* makeStore;
    const running = yield* store.command(
      {
        commandId: "resume-start",
        runId: initial.id,
        expectedRevision: 0,
        event: { type: "enter" },
        now: 0,
    ...
    const interrupted = yield* store.command(
      {
        commandId: `server-interrupted:${running.id}:${running.revision}`,
        runId: running.id,
        expectedRevision: running.revision,
        event: { type: "server_interrupted" },
        now: 5_000_000,
      },
      definition,
    );
    const request = {
      ...
      event: { type: "resume" as const },
      now: 8_000_000,
    };
    const resumed = yield* store.command(request, definition);
    assert.deepStrictEqual(yield* store.command(request, definition), resumed);
    assert.equal(
      (yield* store.get(running.id)).actions.at(-1)?.predecessorActionId,
      running.actions[0]!.id,
    );
    const stale = yield* Effect.exit(
      store.command({ ...request, commandId: "stale-resume" }, definition),
    );
    assert.isTrue(Exit.isFailure(stale));
  }).pipe(Effect.provide(NodeSqliteClient.layerMemory())),
);
