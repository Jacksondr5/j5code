...
export type Event =
  ...
  | { readonly type: "server_interrupted" }
...
export const actionId = (run: Run, task: string, attempt: number): string =>
  `pb:${hash([run.id, run.phase, run.revision, task, attempt])}`;
...
export function decide(
  ...
  if (
    ...
    return {
      ...blocked(
        previous,
        "Pinned playbook definition is unavailable or changed",
        "restore_definition",
        "definition_mismatch",
      ),
      gate: previous.gate,
  ...
  if (event.type === "recover")
    return previous.recovery === "restore_definition"
      ...
      : previous;
  if (event.type === "server_interrupted") {
    if (previous.status !== "running") return previous;
    const actionIds = previous.actions
      .filter(
        (action) =>
          action.phase === previous.phase && ["pending", "claimed"].includes(action.status),
      )
      .map((action) => action.id);
    const generation = (previous.interruptions?.at(-1)?.generation ?? 0) + 1;
    return {
      ...blocked(
        {
          ...previous,
          revision: previous.revision + 1,
          actions: previous.actions.map((action) =>
            actionIds.includes(action.id) ? { ...action, status: "blocked" as const } : action,
          ),
          interruptions: [
            ...(previous.interruptions ?? []),
            { generation, interruptedAt: now, actionIds },
          ],
        },
        "The server stopped before this playbook step completed",
        "resume",
        "server_interrupted",
        actionIds[0] ?? null,
      ),
  ...
  if (event.type === "resume") {
    const interruption = previous.interruptions?.at(-1);
    if (
      previous.status !== "blocked" ||
      previous.recovery !== "resume" ||
      previous.failureCategory !== "server_interrupted" ||
      !interruption ||
      interruption.resumedAt !== undefined
    )
      throw new Conflict("This playbook is not awaiting Resume");
    const run = { ...previous, revision: previous.revision + 1 };
    const predecessors = interruption.actionIds.flatMap((id) => {
      const action = previous.actions.find((candidate) => candidate.id === id);
      return action ? [action] : [];
    });
    const replacements: Action[] = predecessors.map((action) => {
      const { externalIdentity, ...saved } = action;
      return {
        ...saved,
        ...(action.kind === "code" && externalIdentity ? { externalIdentity } : {}),
        id: `pb:${hash([run.id, "resume", interruption.generation, action.id])}`,
    ...
    return {
      ...
      relevantActionId: null,
      recovery: null,
      actions: [...run.actions, ...replacements],
      interruptions: [
        ...(run.interruptions ?? []).slice(0, -1),
        { ...interruption, resumedAt: now },
      ],
    };
  }
