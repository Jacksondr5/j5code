...
import {
  ...
  RunStatus,
  StartRequest,
  PlaybookDefinitionImportRequest,
  PlaybookDefinitionStateRequest,
...
const decodeDefinitionImportEffect = Schema.decodeUnknownEffect(PlaybookDefinitionImportRequest);
const decodeDefinitionStateEffect = Schema.decodeUnknownEffect(PlaybookDefinitionStateRequest);
...
export const playbookHttpLayer = Layer.unwrap(
  Effect.gen(function* () {
    const service = yield* PlaybookService;
    const sql = yield* SqlClient.SqlClient;
    const handler = (operate: boolean) =>
      Effect.gen(function* () {
        const session = yield* authenticate(operate);
        const request = yield* HttpServerRequest.HttpServerRequest;
        const url = new URL(request.url, "http://playbook.local");
        ...
        if (!operate) {
          ...
          if (id === "approval-count")
            return HttpServerResponse.jsonUnsafe({
              count: yield* readPlaybookApprovalCount().pipe(
                Effect.provideService(SqlClient.SqlClient, sql),
              ),
            });
          if (id === "thread-parent")
            return HttpServerResponse.jsonUnsafe({
              parent: yield* readPlaybookThreadParent(url.searchParams.get("threadId") ?? "").pipe(
                Effect.provideService(SqlClient.SqlClient, sql),
              ),
            });
          if (id === "definitions")
        ...
        Effect.catchTags({
          ...
          SchemaError: () =>
            Effect.succeed(
              HttpServerResponse.jsonUnsafe(
                { message: "Invalid playbook request" },
                { status: 400 },
              ),
            ),
    ...
    return Layer.mergeAll(
      HttpRouter.add("GET", "/api/j5/playbooks", handler(false)),
      HttpRouter.add("GET", "/api/j5/playbooks/board", handler(false)),
      HttpRouter.add("GET", "/api/j5/playbooks/:id/timeline", handler(false)),
      HttpRouter.add("GET", "/api/j5/playbooks/:id", handler(false)),
      HttpRouter.add("GET", "/api/j5/playbooks/:id/artifacts/:artifactId", handler(false)),
      HttpRouter.add("POST", "/api/j5/playbooks", handler(true)),
      HttpRouter.add("POST", "/api/j5/playbooks/:id/:action", handler(true)),
    );
  }),
);
