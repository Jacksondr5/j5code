...
export const playbookMigrations = {
  ...
  "2_PlaybookResumeActions": Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    yield* sql`ALTER TABLE j5_playbook_actions ADD COLUMN predecessor_action_id TEXT`;
    yield* sql`CREATE INDEX j5_playbook_actions_predecessor
      ON j5_playbook_actions(predecessor_action_id) WHERE predecessor_action_id IS NOT NULL`;
  }),
};
