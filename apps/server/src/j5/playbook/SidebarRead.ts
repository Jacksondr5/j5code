import { PlaybookEntry } from "@j5/playbook-contracts/sidebar";
import * as Effect from "effect/Effect";
import * as Schema from "effect/Schema";
import * as SqlClient from "effect/unstable/sql/SqlClient";

const decodeEntries = Schema.decodeUnknownEffect(Schema.Array(PlaybookEntry));
...
export const readPlaybookEntries = Effect.fn("Playbook.readEntries")(function* (
  squadronId: string,
  query: string,
  offset: number,
  limit = 50,
...
export const readPlaybookApprovalCount = Effect.fn("Playbook.readApprovalCount")(function* () {
  const sql = yield* SqlClient.SqlClient;
  const rows = yield* sql<{ count: number }>`SELECT count(*) AS count FROM j5_playbook_runs
    WHERE status='waiting_approval'`;
  return rows[0]?.count ?? 0;
});

export const readPlaybookThreadParent = Effect.fn("Playbook.readThreadParent")(function* (
  threadId: string,
) {
  const sql = yield* SqlClient.SqlClient;
  const rows = yield* sql<{ runId: string; squadronId: string; title: string }>`SELECT
    ...
    FROM j5_playbook_actions a JOIN j5_playbook_runs r ON r.id=a.run_id
