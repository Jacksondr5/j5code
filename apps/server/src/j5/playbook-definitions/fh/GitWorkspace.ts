...
export async function resolveBase(repository: string, baseRef: string): Promise<string> {
  ...
  if (!ref) throw new Error("Enter a base ref before starting the playbook");
  try {
...
export async function publicationBaseBranch(repository: string, baseRef: string): Promise<string> {
  ...
  throw new Error("HEAD is detached; enter an explicit base branch before starting the playbook");
}
...
export async function workspace(repository: string, baseCommit: string, root: string, id: string) {
  const branch = `j5/playbook-${hash(id).slice(0, 20)}`;
  ...
  if (!(await exists(worktree))) {
    ...
    if (existingBranch.exitCode === 0) {
      if ((await git(repository, ["rev-parse", branch])) !== baseCommit)
        throw new Error("Playbook branch was changed before worktree recovery");
  ...
  if (
    ...
    throw new Error("Playbook worktree does not match the pinned base and branch");
  }
