...
const developmentInputs = Schema.decodeUnknownSync(DevelopmentInputs);
...
const definition: Omit<Definition, "hash"> = {
  ...
  input: (run, phase, task) => {
    ...
    return {
      personaId,
      assignmentDigest: readPlaybookExecution(run.execution).personas[personaId].definitionDigest,
      worktree: workspace.worktree,
      branch: workspace.branch,
      selectedEvidenceIds,
      selectedEvidenceHashes,
      prompt: `Playbook operating instructions v${INSTRUCTIONS_VERSION}. ${instructions[instructionId]}\nReturn ONLY one JSON object. Do not invoke other agents or change Git history.\n${canonical(
        {
...
export const development: Definition = { ...definition, hash: definitionHash };
