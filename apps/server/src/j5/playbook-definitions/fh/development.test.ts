...
import { gateHash, hash } from "../../playbook/Definition.ts";
import {
  git,
  candidate,
  command,
...
const playbookPhase = (id: string) => development.phases.find((phase) => phase.id === id)!;
...
async function fixture() {
  ...
  await git(repository, ["config", "user.name", "Playbook Fixture"]);
  await git(repository, ["config", "user.email", "playbook@example.test"]);
...
it.each([
  ...
  assert.equal(
    development.outcome(run, playbookPhase("plan_review"), [
...
it("revises rejected code reviews regardless of plan counters", () => {
  const run = testRun({ plan: 3, plan_review: 3 });
  assert.equal(
    development.outcome(run, playbookPhase("code_review"), [
...
it("bounds diagnosis, correction, build, and validation visits", () => {
  assert.equal(playbookPhase("build").maxVisits, 3);
  assert.equal(playbookPhase("validation").maxVisits, 4);
  assert.equal(playbookPhase("verification_diagnosis").maxVisits, 5);
  assert.equal(playbookPhase("checks_approval").maxVisits, 2);
  assert.equal(
    development.outcome(correctionRun(), playbookPhase("verification_diagnosis"), [
      {
        id: "environment",
        phase: "verification_diagnosis",
        content: {
...
it("reports an actionable error when the base ref is unavailable", async () => {
  ...
  assert.equal(
    ...
    "Error: Enter a base ref before starting the playbook",
  );
