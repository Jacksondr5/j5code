...
import { hash, restartPresentation, validateDefinition } from "../playbook/Definition.ts";
...
export const makeService = Effect.gen(function* () {
  ...
  while (true) {
    const records = yield* store.runningBatch(recoveryCursor);
    if (records.length === 0) break;
    for (const record of records) {
      const definition = definitions.find(
        (item) =>
          item.id === record.definitionId &&
          item.version === record.definitionVersion &&
          item.hash === record.definitionHash,
      );
      yield* store.command(
        {
          commandId: `server-interrupted:${record.id}:${record.revision}`,
          runId: record.id,
    ...
    recoveryCursor = records.at(-1)!.creationSequence;
  }
