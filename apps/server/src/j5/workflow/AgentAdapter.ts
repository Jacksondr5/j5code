...
export const makeAgentAdapter = Effect.gen(function* () {
  ...
  return {
    ...
    reconcile: (action, run, recordIdentity) =>
      Effect.gen(function* () {
        if (!hasWorkflowSnapshots(run.execution))
          return { status: "blocked", cause: legacyWorkflowMessage, recovery: null } as const;
        ...
        if (!projection?.runs.some((item) => item.userMessageId === messageId)) {
          ...
          if (
            ...
            return {
              status: "blocked",
              cause: "Workflow persona snapshot reference is invalid. Start a fresh task.",
              recovery: null,
            } as const;
