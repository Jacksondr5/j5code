...
export const YamlWorkflow = Schema.Struct({
  schema: Schema.Literal("t3-workflow/v1"),
  id: NonEmpty,
...
export type YamlWorkflow = typeof YamlWorkflow.Type;
const decodeYamlWorkflow = Schema.decodeUnknownSync(YamlWorkflow);

export class WorkflowDefinitionSourceError extends Error {
...
function sourceValue(text: string, file: string): unknown {
  try {
    ...
    throw new WorkflowDefinitionSourceError(file, "$", String(cause));
  }
}
...
const selectedEvidenceHashes = (input: unknown): readonly string[] => {
...
export function compileYamlWorkflow(
  text: string,
  file = "workflow.yaml",
  implementations: Readonly<Record<string, Definition>> = {},
): Definition {
  let source: YamlWorkflow;
  try {
    source = decodeYamlWorkflow(sourceValue(text, file));
  } catch (cause) {
    if (cause instanceof WorkflowDefinitionSourceError) throw cause;
    throw new WorkflowDefinitionSourceError(file, "$", String(cause));
  }
  if (source.phases.length === 0)
    throw new WorkflowDefinitionSourceError(file, "phases", "at least one phase is required");
  if (source.implementation) {
    const registered = implementations[source.implementation];
    if (!registered)
      throw new WorkflowDefinitionSourceError(
        file,
        "implementation",
    ...
    return {
      ...
      runtime: `${source.implementation}:compat`,
  ...
  const definition: Omit<Definition, "hash"> = {
    ...
    input: (run, phase, task) => {
      ...
      const execution = readWorkflowExecution(run.execution);
  ...
  try {
    ...
    throw new WorkflowDefinitionSourceError(file, "phases", String(cause));
  }
  return compiled;
}
