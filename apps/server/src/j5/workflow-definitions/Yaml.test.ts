...
it("compiles ordered phases and explicit shared agents", () => {
  const definition = compileYamlWorkflow(source, "research-review.yaml");
...
it("loads Development YAML through its pinned compatibility implementation", () => {
  ...
  const compiled = compileYamlWorkflow(yaml, "development.yaml", {
...
it("reports source files and fields for invalid references", () => {
  assert.throws(
    () => compileYamlWorkflow(source.replace("agent: researcher", "agent: missing"), "team.yaml"),
    /team\.yaml:phases\.research\.tasks\.investigate\.agent: unknown agent missing/,
  );
  assert.throws(
    () =>
      compileYamlWorkflow(source.replace("completed: review", "completed: nowhere"), "team.yaml"),
...
it("maps registered code operations without permitting scripts", () => {
  ...
  const definition = compileYamlWorkflow(withValidation, "operations.yaml");
...
it("rejects scheduling the same shared conversation twice in one phase", () => {
  ...
  assert.throws(
    () => compileYamlWorkflow(duplicate, "team.yaml"),
