...
import {
  makeSkillLinkRpcHandlers,
  resolveSkillLinkRoot,
  skillLinkDiscovery,
} from "./skillLinkRpc.ts";
    ...
    () =>
      run(
        Effect.gen(function* () {
          const f = yield* fixture;
          const homeVar = (yield* HostProcessPlatform) === "win32" ? "USERPROFILE" : "HOME";
          const codex = yield* resolveSkillLinkRoot(
            {
              driver: ProviderDriverKind.make("codex"),
              config: {
          ...
          assert.equal(
            yield* resolveSkillLinkRoot(claude, "user", f.cwd),
            f.path.join(f.cwd, "relative-home", "skills"),
          );
          assert.equal(
            yield* resolveSkillLinkRoot(
