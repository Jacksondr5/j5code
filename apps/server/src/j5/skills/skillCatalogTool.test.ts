...
describe("skill catalog tool", () => {
  test("reads descriptions eight at a time and preserves catalog group and skill order", (f) =>
    Effect.gen(function* () {
      const names = Array.from({ length: 17 }, (_, index) => `skill-${16 - index}`);
      const groups = {
        zulu: { description: "First", skills: names.slice(0, 9) },
        alpha: { description: "Second", skills: names.slice(9) },
      };
      yield* f.fs.writeFileString(f.path.join(f.catalogDir, "catalog.yaml"), stringify({ groups }));
      const gates = names.map(() => Promise.withResolvers<string>());
      const started = names.map(() => Promise.withResolvers<void>());
      let active = 0;
      let peak = 0;
      const descriptions = vi
        .spyOn(Installer, "skillDescription")
        .mockImplementation(async (_, skill) => {
          const index = names.indexOf(skill);
      ...
      const fiber = yield* f.tool().status({ source: f.catalogDir }).pipe(Effect.forkChild);
      for (let offset = 0; offset < names.length; offset += 8) {
        yield* Effect.promise(() =>
          Promise.all(started.slice(offset, offset + 8).map((entry) => entry.promise)),
        );
        for (let index = Math.min(offset + 8, names.length) - 1; index >= offset; index--)
          gates[index]!.resolve(`Description ${names[index]}`);
      }
      const result = yield* Fiber.join(fiber);
      assert.equal(peak, 8);
      assert.equal(descriptions.mock.calls.length, 17);
  ...
  test("overlaps Git status and upstream reads but suppresses upstream warnings after status failure", (f) =>
    Effect.gen(function* () {
      ...
      for (const failStatus of [false, true]) {
        ...
        const tool = f.tool(
          f.runner((input) => {
            if (input.args.includes("--show-toplevel")) return Effect.succeed(output(f.catalogDir));
            if (input.args[0] === "status")
              return Deferred.await(upstreamStarted).pipe(
                Effect.as(failStatus ? output("", 128) : output(" M catalog.yaml")),
              );
            return Deferred.succeed(upstreamStarted, undefined).pipe(
              Effect.andThen(
                Effect.fail(
                  new ProcessRunner.ProcessSpawnError({
                    command: "git",
                    argumentCount: 4,
                    cause: "upstream denied",
                  }),
                ),
              ),
            );
        ...
        assert.deepStrictEqual(result.git, { upstream: null, dirty: !failStatus });
        assert.equal(result.warnings.length, 2);
        assert.equal(result.warnings[0], 'Saved group "missing" is not in the catalog.');
        assert.match(
          result.warnings[1]!,
          failStatus ? /^Git status failed:/ : /^Git upstream check failed:/,
        );
      }
    }));
    ...
    Effect.gen(function* () {
      for (const mode of ["non-repo", "dirty", "no-upstream", "ok", "fetch-fails", "pull-fails"]) {
            ...
            Effect.succeed(
              input.args[0] === "status"
                ...
                : input.args[0] === "rev-parse"
                  ...
                  : input.args[0] === "pull" && (mode === "fetch-fails" || mode === "pull-fails")
        ...
        if (mode === "ok") {
          ...
          assert.equal(
            f.calls.length,
            mode === "non-repo" || mode === "dirty" ? 1 : mode === "no-upstream" ? 2 : 3,
          );
        }
      }
    }));
