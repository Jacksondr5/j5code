...
// converges and a hand-edited or pre-existing file is simply treated as version 1.
// ---------------------------------------------------------------------------

const VERSIONED_MARKER_PREFIX = "<!-- j5-artifact-versions: ";
const VERSION_HEADING = /^## Version (\d+) · (\S+)(?: · current)?\s*$/;
...
export const artifactVersionedMarker = (relativePath: string) =>
  `${VERSIONED_MARKER_PREFIX}${relativePath} -->`;

/** Versions newest first; a file without the marker is one unnumbered version of everything it holds. */
export function parseArtifactVersions(
  ...
  if (!lines[0]?.startsWith(VERSIONED_MARKER_PREFIX)) {
    return [{ number: 1, timestamp: now, content: existing.trim() }];
  }
  const versions: Array<ArtifactVersion & { lines: string[] }> = [];
  for (const line of lines) {
    const heading = VERSION_HEADING.exec(line);
    if (heading) {
      versions.push({ number: Number(heading[1]), timestamp: heading[2]!, content: "", lines: [] });
      continue;
    }
    versions.at(-1)?.lines.push(line);
  }
  return versions.map(({ number, timestamp, lines }) => {
    const body = lines.join("\n").trim();
    return { number, timestamp, content: body.endsWith("\n---") ? body.slice(0, -3).trim() : body };
  });
}
...
export function renderArtifactVersions(input: {
  ...
  const sections = input.versions.map(
    (version, index) =>
      `## Version ${version.number} · ${version.timestamp}${index === 0 ? " · current" : ""}\n\n${version.content}\n`,
  );
  return `${artifactVersionedMarker(input.relativePath)}\n# ${title}\n\n${summary}\n\n${sections.join("\n---\n\n")}`;
}
