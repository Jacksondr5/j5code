...
The future A2S `archive_agent` operation is expected to compose these public service boundaries directly: `ArchiveFactsService` supplies the AR2 preflight read and `LifecycleService.archiveParticipant` supplies the same idempotent R1 termination engine used by the lifecycle reactor. A9 does not ship an `archive_agent` handler or tool stub.
