...
it.effect("spawns a saved agent as a Peer Agent only within its declared routes", () =>
  Effect.gen(function* () {
    ...
    const commands = yield* Ref.make<ReadonlyArray<OrchestrationV2Command>>([]);
    const parentAssignment = yield* Ref.make<OrchestrationV2AgentPersonaAssignment | undefined>(
      undefined,
    );
    ...
    const dependencies = Layer.mergeAll(
      ...
      Layer.mock(ThreadManagementService)({
        getThreadProjection: (threadId) =>
          Ref.get(parentAssignment).pipe(
            Effect.map((assignment) => {
              const base = projection(threadId);
              return threadId === invocation.threadId && assignment !== undefined
                ? ({
                    ...base,
                    thread: { ...base.thread, agentPersonaAssignment: assignment },
    ...
    yield* Effect.gen(function* () {
      ...
      yield* Ref.set(parentAssignment, {
        personaId: "scout",
        definitionVersion: BUILT_IN_AGENT_PERSONAS.scout.version,
        authorityPolicy: "read-only",
        resolvedRoute: "primary",
        resolvedDriver: ProviderDriverKind.make("codex"),
        resolvedModelSelection: expectedSelection,
      });
      const escalation = yield* call({
        ...scout,
        agent: "builder",
        model: "gpt-5.6-sol",
        client_request_id: "spawn-persona-escalation",
      });
      assert.isTrue(escalation.isFailure);
      assert.include(
        failureMessage(escalation),
        "A read-only agent cannot spawn an agent with write access",
      );
      assert.lengthOf(yield* Ref.get(commands), commandCount);
      const readOnlyChild = yield* call({ ...scout, client_request_id: "spawn-persona-read-only" });
      assert.isFalse(readOnlyChild.isFailure, failureMessage(readOnlyChild));
    }).pipe(Effect.provide(layer));
  }),
);
