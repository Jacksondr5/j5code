...
/**
 * A pinned runtime is an exact `@jacksondr5/j5code@<version>` npm-installed into
 ...
 * switching over, never `npx @jacksondr5/j5code`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "@jacksondr5/j5code", "dist", "bin.mjs"),
...
/**
 * Installs `@jacksondr5/j5code@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "@jacksondr5/j5code", "dist", "bin.mjs"),
    sentinelPath: input.path.join(stagingDir, ".install-complete"),
  };

  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned j5 runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
          ...
          "--no-fund",
          "--no-audit",
          `@jacksondr5/j5code@${input.version}`,
        ],
