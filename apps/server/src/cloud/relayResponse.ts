...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    ...
    case "RelayAuthInvalidError":
      return "Run `j5code connect login` to check this machine's authorization. If the stored credential was revoked, sign out with `j5code connect logout`, then run `j5code connect` again. Restart T3 Code after signing in.";
