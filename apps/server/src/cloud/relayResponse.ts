...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    ...
    case "RelayAuthInvalidError":
      return "Run `j5 connect login` to check this machine's authorization. If the stored credential was revoked, sign out with `j5 connect logout`, then run `j5 connect` again. Restart T3 Code after signing in.";
