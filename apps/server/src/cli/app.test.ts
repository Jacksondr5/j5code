...
describe("j5code app", () => {
    ...
    withTempDirectory("t3-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`j5code app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
    ...
    withTempDirectory("t3-app-platform-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`j5code app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
  ...
  it.effect("uses J5CODE_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("t3-app-command-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"], { J5CODE_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("t3-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        ...
        const baseDir = NodePath.join(root, ".j5code");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("never searches a dev state directory for an explicit T3 home", () =>
    withTempDirectory("t3-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        ...
        const envError = yield* runCli(["app"], { J5CODE_HOME: baseDir }).pipe(Effect.flip);
