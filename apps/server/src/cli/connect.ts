...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `j5code connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `j5code connect link` to enable T3 Connect."
      : !status.linked
  ...
  return [
    ...
    "",
    "This is saved setup, not a live connection check. Check the background service with `j5code service status`.",
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `j5code connect unlink` again when the relay is reachable.",
      );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      "Signed out of T3 Connect locally.\nThe background service is managed separately with `j5code service`.",
    );
  }
});
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `j5code connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `j5code connect link` before the
