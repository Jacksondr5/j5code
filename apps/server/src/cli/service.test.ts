...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `j5 service install` to repair it.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("Run `j5 service install` to repair it.");
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(output, "j5@0.0.32-nightly.1 (newer than this j5@0.0.31 CLI)");
  assert.include(output, "Run `j5 update 0.0.32-nightly.1` to match it");
