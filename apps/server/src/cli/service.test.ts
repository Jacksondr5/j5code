...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "J5 Code service",
      "  Status: installed · j5code@0.0.29",
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx j5code@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx j5code@0.0.32-nightly.1 service update");
  expect(output).not.toContain("j5code@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx j5code@0.0.29 service update");
  expect(output).not.toContain("npx j5code@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "j5code@0.0.32-nightly.1 (newer than this j5code@0.0.31 CLI)");
  assert.include(output, "npx j5code@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx j5code@latest service update");
});
