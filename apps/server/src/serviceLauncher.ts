...
// `j5code service update`. Keep runtime imports limited to Node built-ins.
...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: NodePath.join(versionDir, "node_modules", "j5code", "dist", "bin.mjs"),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected j5code@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
