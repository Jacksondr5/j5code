...
export function SkillInstallerSettings() {
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection
        title="Skills"
        description="Install shared skill groups from a catalog into this environment. No agent chat involved."
      >
        <div className="grid gap-4 p-4">
          ...
          {effectiveEnvironmentId === null ? (
            ...
            <SkillCatalogPanel
              ...
              environmentLabel={selectedEnvironment?.label ?? effectiveEnvironmentId}
            />
          )}
        </div>
      </SettingsSection>
...
export function SkillCatalogPanel({
  ...
  // Catalog identity derives from acknowledged settings: every
  // status/apply/update call sends the live configured source, and
  // `loadedSource` records which source the displayed status was fetched
  // for. A cached source must never pin requests after settings move.
  const querySource = configuredSource;
  const status = useEnvironmentQuery(
    skillCatalogEnvironment.status({ environmentId, input: { expectedSource: querySource } }),
  );
  const [loadedSource, setLoadedSource] = useState<string | null>(null);
  const statusSucceeded = !status.isPending && status.data !== null;
  useEffect(() => {
    setLoadedSource((previous) => loadedSourceAfterStatus(previous, querySource, statusSucceeded));
  }, [statusSucceeded, querySource]);
  ...
  const selectedGroups = pruneSelectedGroups(
    selection.scope === selectionScope ? selection.groups : initialSelection(status.data),
    knownGroups,
  );
  ...
  async function saveSource() {
    ...
    try {
      ...
      // until the new source's status arrives and `loadedSource` catches up.
      status.refresh();
    } finally {
      setBusy(false);
    }
  ...
  async function applySelected() {
    ...
    try {
      const result = await applyGroupsCommand({
        environmentId,
        input: { expectedSource: querySource, groups: selectedGroups },
      });
  ...
  async function updateCheckout() {
    ...
    try {
      const result = await updateCatalogCommand({
        environmentId,
        input: { expectedSource: querySource },
      });
      if (selectionScopeRef.current !== originScope) return;
      if (result._tag === "Failure") {
        const cause = squashAtomCommandFailure(result);
        ...
        <>
          ...
          <fieldset className="grid gap-2" disabled={applyDisabled}>
            ...
            {status.data.groups.map((group) => (
              <label key={group.name} className="flex cursor-pointer gap-2 text-sm">
                <input
                  type="checkbox"
