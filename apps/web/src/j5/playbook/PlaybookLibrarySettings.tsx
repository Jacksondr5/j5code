...
export function PlaybookLibrarySettings() {
  ...
  const refresh = useCallback(
    () =>
      primaryEnvironmentId === null
        ? Promise.resolve()
        : listPlaybookDefinitions(primaryEnvironmentId)
            .then(setDefinitions)
  ...
  if (primaryEnvironmentId === null) return <p>Connect an environment to manage Playbooks.</p>;
  return (
    <SettingsPageContainer>
        ...
        <input
          ...
          onChange={(event) => {
            ...
            void run(async () => {
              ...
              try {
                return await importPlaybookDefinitions(primaryEnvironmentId, contents);
              } catch (cause) {
                ...
                if (!window.confirm(`${errorMessage(cause)}\n\nReplace the existing definitions?`))
                  return definitions;
                return importPlaybookDefinitions(primaryEnvironmentId, contents, true);
              }
            });
          }}
        />
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (
                            ...
                            return;
                          void run(() =>
                            removePlaybookDefinition(primaryEnvironmentId, definition.id),
                          );
                        }}
                      >
                        Remove
