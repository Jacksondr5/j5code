...
it("forces overview for approval links and rejects invalid filters", () => {
  assert.equal(effectiveTab({ tab: "timeline" }, "#playbook-approval"), "overview");
...
it("keeps an approval link scoped to its environment", () => {
  const search = { environmentId: "remote", runId: "run" };
  assert.deepEqual(serializeRunsSearch(parseRunsSearch(search)), search);
});
