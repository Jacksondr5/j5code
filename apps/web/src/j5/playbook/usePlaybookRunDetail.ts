...
import type { RunDetail } from "@j5/playbook-contracts";
...
export type PendingPlaybookAction =
  | "approve"
  | "request_changes"
  | "cancel"
  | "recover"
...
export const shouldApplyPlaybookMutation = (
  originSelection: string,
  currentSelection: string,
  responseRunId: string,
) => currentSelection === originSelection && responseRunId === originSelection;

export function usePlaybookRunDetail(environmentId: EnvironmentId, runId: string) {
  ...
  const detailQuery = usePlaybookQuery(playbookDetailAtom({ environmentId, input: { runId } }));
  const definitionsQuery = usePlaybookQuery(playbookDefinitionsQuery(environmentId));
  ...
  const submit = useCallback(
    async (
      ...
      try {
        const next = await mutateRun(environmentId, path, {
          ...body,
        ...
        if (shouldApplyPlaybookMutation(origin, selectedRef.current, next.id)) {
          setMutationRun(next);
          setFeedback(null);
          if (action === "save") {
            const saved = {
