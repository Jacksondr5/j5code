...
export const playbookTimelineQuery = Atom.family((key: string) => {
  ...
  const atom = Atom.make(
    Effect.promise(async () =>
      retainTimelinePage(
        key,
        await readPlaybookTimeline(target.environmentId, target.input.runId, target.input.before),
      ),
    ),
  ).pipe(
    Atom.swr({
  ...
  return markPollable(
    Atom.make(
      Effect.promise(async () => {
        ...
        const next = await readRun(target.environmentId, target.input.runId, previous?.readVersion);
...
export const playbookDefinitionsQuery = Atom.family((environmentId: EnvironmentId) =>
  Atom.make(Effect.promise(() => listPlaybookDefinitions(environmentId))).pipe(
...
export const playbookApprovalCountQuery = Atom.family((environmentId: EnvironmentId) =>
  markPollable(
    Atom.make(Effect.promise(() => readPlaybookApprovalCount(environmentId))).pipe(
      Atom.swr({ staleTime: 5_000, revalidateOnMount: true }),
      Atom.setIdleTTL(5 * 60_000),
