...
export function PeerServersSettings({
  ...
  return (
    <SettingsSection
      title="Peer servers"
      description="Other J5 servers whose agents can exchange messages with agents here. Agents address each other by participant id; which server a Squadron lives on stays the platform's business."
      headerAction={
        <Button
          size="xs"
          variant="ghost"
    ...
    >
      ...
      {peers.data !== null && peers.data.length === 0 ? (
        <SettingsRow
          title="No peers yet"
          description="Peer this server with another environment you are connected to so their agents can exchange messages."
        />
      ) : null}
      {peers.error !== null ? (
