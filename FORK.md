...
22. B4's typed confirmation-content seam: `apps/web/src/confirmDialog.ts:4-23,84-145` extends only the in-memory `requestConfirmDialog(message, options, content?)` coordinator state with an optional `ReactNode`; `apps/web/src/components/ConfirmDialogHost.tsx:55-66,77-106` renders that typed node when present and otherwise keeps its existing string title/description path. The host has no J5 import, payload decoding, or private prefix branch. J5 owns the archive row content in `apps/web/src/j5/a2a/ArchiveWarningContent.tsx:6-130`; `apps/web/src/j5/a2a/archiveFlow.ts:13-16,39-88` builds and passes it for consequential archive facts, and `apps/web/src/components/Sidebar.tsx:3169-3179` plus `apps/web/src/components/LegacySidebar.tsx:1824-1836,2009-2016` use the existing confirmation coordinator to carry the slot. Existing string callers omit the third argument and remain byte-for-byte on the prior path. On every rebase, verify the host stays J5-agnostic, the absent-content string branch remains intact, and the three archive doors continue to pass J5-owned content only for fact-backed warnings.
