...
9. SQ1's first-run gate mount: `apps/web/src/routes/_chat.index.tsx:12` imports J5-owned `SquadronFirstRunGate`, and `ChatIndexRouteView` at `:32-36` wraps only the non-hosted-static `IndexDraftLanding` return. The hosted-static onboarding branch at `:28-30` remains unchanged. State and UI stay in `apps/web/src/j5/squadron/`; until the authenticated J5 route aggregate lands, the mounted gate is explicitly `unavailable` and fails closed rather than creating a home-less thread or a default Squadron. On every rebase, verify those import and return anchors still wrap only this branch before retaining the exception.
10. SQ1's lossless launch carrier: `packages/contracts/src/orchestrationV2.ts:2322-2342` adds additive-optional unbranded `squadronId` after `creationSource`; `apps/server/src/orchestration-v2/ThreadLaunchService.ts:58-73` retains it as an optional unbranded input field; and `apps/server/src/ws.ts:1230-1234` forwards it only when present, immediately before the fixed provenance fields. `ThreadLaunchService` does not interpret, default, or validate the field; the future J5 authenticated creation boundary is its sole enforcement owner. On every rebase, verify this exact contract → launch-input → WebSocket passthrough chain, and that no defaulting or behavior was added.
