...
9. SQ1's first-run gate mount and authenticated J5 route aggregate: `apps/web/src/routes/_chat.index.tsx:12` imports J5-owned `SquadronFirstRunGate`, and `ChatIndexRouteView` at `:32-36` wraps only the non-hosted-static `IndexDraftLanding` return. The hosted-static onboarding branch at `:28-30` remains unchanged. State and UI stay in `apps/web/src/j5/squadron/`; until the authenticated route is available, the mounted gate is explicitly `unavailable` and fails closed rather than creating a home-less thread or a default Squadron. In `apps/server/src/server.ts:78,441`, `j5AuthenticatedRoutesLayer` is the one replacement for A4's raw route entry after `websocketRpcRouteLayer`. The J5-owned aggregate in `apps/server/src/j5/a2a/J5AuthenticatedRoutes.ts` composes A4's human inbox with SQ1's Squadron list/create and creation-time launch routes, then provides SQ1's J5-owned services. Future J5 authenticated route layers enter through this aggregate (B6 is the next tenant), never through another `server.ts` entry. On every rebase, verify the web import/return anchors still wrap only this branch and the single server entry still follows `websocketRpcRouteLayer`.
10. SQ1's lossless launch carrier: `packages/contracts/src/orchestrationV2.ts:2322-2342` adds additive-optional unbranded `squadronId` after `creationSource`; `apps/server/src/orchestration-v2/ThreadLaunchService.ts:58-73` retains it as an optional unbranded input field; and `apps/server/src/ws.ts:1230-1234` forwards it only when present, immediately before the fixed provenance fields. `ThreadLaunchService` does not interpret, default, or validate the field; the J5 authenticated creation boundary is its sole enforcement owner. On every rebase, verify this exact contract → launch-input → WebSocket passthrough chain, and that no defaulting or behavior was added.
