...
6. A4's visible human-inbox navigation append, relocated by B1: `apps/web/src/components/sidebar/SidebarChrome.tsx`, where `SidebarChromeFooter` still recognizes `/inbox` for Back behavior, while the original footer Inbox item at the A4 anchor was removed and `SidebarChromeHeader` now appends the J5-owned bell + live-count link immediately after `SidebarBrand`. The route, page, bell, and count client remain under J5-owned files; Settings, Pull Requests, Usage, and Back behavior are unchanged.
