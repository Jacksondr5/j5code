...
if command -v j5 >/dev/null 2>&1; then
  exec j5 "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (j5 depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a j5 executable. \`npx --yes\` then exits 0 without
...
require_installed_t3_cli() {
  if ! T3_CLI_PATH="$("$@" -- sh -c 'command -v j5')"; then
  ...
  printf 'Remote host installed %s but npm produced no j5 executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
...
printf 'Remote host is missing the j5 CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@jacksondr5/j5code@latest");
