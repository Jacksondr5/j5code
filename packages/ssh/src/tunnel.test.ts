...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(script, 'exec j5 "$@"');
    assert.include(script, 'exec "$T3_CLI_PATH" "$@"');
    assert.include(script, "could not install '@jacksondr5/j5code@latest'");
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --package '@jacksondr5/j5code@latest'",
    );
    assert.include(
      script,
      "require_installed_t3_cli npm exec --yes --package '@jacksondr5/j5code@latest'",
    );
    assert.include(script, "npm produced no j5 executable");
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    const script = buildRemoteT3RunnerScript({
      packageSpec: "@jacksondr5/j5code@nightly; touch /tmp/t3-owned",
    });

    assert.include(
      script,
      "require_installed_t3_cli npx --yes --package '@jacksondr5/j5code@nightly; touch /tmp/t3-owned'",
    );
    assert.notInclude(script, "exec npx --yes @jacksondr5/j5code@nightly; touch /tmp/t3-owned");
  });
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(j5LaunchScript, 'DEFAULT_SERVER_HOME="$HOME/.j5code"');
    assert.notInclude(j5LaunchScript, 'DEFAULT_SERVER_HOME="$HOME/.t3"');
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "@jacksondr5/j5code@nightly" }),
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "@jacksondr5/j5code@nightly" }),
      "@jacksondr5/j5code@nightly",
    );
    assert.include(
