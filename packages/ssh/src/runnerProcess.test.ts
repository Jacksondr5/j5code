    ...
    it.live.each(["npx", "npm"] as const)(
      ...
      (packageManager) =>
        Effect.gen(function* () {
          ...
          const packageSpec = "@jacksondr5/j5code@0.0.35";
  ...
  () => {
    ...
    it.live.each(cases)("handles $packageManager/$mode", ({ packageManager, mode }) =>
      Effect.gen(function* () {
        ...
        const packageSpec = "@jacksondr5/j5code@0.0.39";
...
`,
        ...
        yield* fs.chmod(path.join(bin, packageManager), 0o700);
        if (mode === "existing-cli") yield* fs.symlink(cliPath, path.join(bin, "j5"));
        ...
        if (installFailed) {
          ...
          assert.include(stderr, "npm produced no j5 executable");
